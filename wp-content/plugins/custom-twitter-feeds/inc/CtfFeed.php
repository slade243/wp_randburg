...
class CtfFeed
{
     ...
     * @var mixed|void
     */
    protected $db_configure;

    /**
     * @var mixed|void
     */
    protected $db_customize;
    ...
    protected function setFeedOptions()
    {
        ...
        $bool_true_configure = array (
            'have_own_tokens',
            'include_replies',
            'ajax_theme'
        );
        $this->setStandardBoolOptions( $bool_true_configure, 'configure', false );

        $this->setAccessTokenAndSecretOptions();
        $this->setConsumerKeyAndSecretOptions();

        $db_only_customize =  array(
            'request_method'
        );
        $this->setDatabaseOnlyOptions( $db_only_customize, 'customize' );

        $this->setStandardTextOptions( 'num', 'configure', 5 );

        $standard_text_custom = array(
            'class',
            'headertext',
            'dateformat',
            'datecustom',
        ...
        $bool_true_custom = array(
            'showbutton'
        );
        $this->setStandardBoolOptions( $bool_true_custom, 'customize' );

        $bool_false_custom = array(
        ...
        $this->setStandardBoolOptions( $bool_false_custom, 'customize', false );

        $this->setStandardBoolOptions( 'showbio', 'style', true );
    ...
    protected function setConsumerKeyAndSecretOptions()
    {
        if ( $this->feed_options['have_own_tokens'] ) {
            $this->feed_options['consumer_key'] = isset( $this->db_configure['consumer_key'] ) && strlen( $this->db_configure['consumer_key'] ) > 15 ? $this->db_configure['consumer_key'] : 'FPYSYWIdyUIQ76Yz5hdYo5r7y';
            $this->feed_options['consumer_secret'] = isset( $this->db_configure['consumer_secret'] ) && strlen( $this->db_configure['consumer_secret'] ) > 30 ? $this->db_configure['consumer_secret'] : 'GqPj9BPgJXjRKIGXCULJljocGPC62wN2eeMSnmZpVelWreFk9z';
        } else {
     ...
     */
    public function setDatabaseOnlyOptions( $options, $options_page, $default = '' )
    {
        if ( is_array( $options ) ) {
            foreach ( $options as $option ) {
                switch ( $options_page ) {
                    case 'configure':
                        $this->feed_options[$option] = isset( $this->db_configure[$option] ) && ! empty( $this->db_configure[$option] ) ? $this->db_configure[$option] : $default;
                        break;
                    case 'customize':
                        $this->feed_options[$option] = isset( $this->db_customize[$option] ) && ! empty( $this->db_customize[$option] ) ? $this->db_customize[$option] : $default;
                        break;
                    default:
                        $this->feed_options[$option] = isset( $this->db_style[$option] ) && ! empty( $this->db_style[$option] ) ? $this->db_style[$option] : $default;
                        break;
                }
            }
        } else {
            switch ( $options_page ) {
                case 'configure':
                    $this->feed_options[$options] = isset( $this->db_configure[$options] ) && ! empty( $this->db_configure[$options] ) ? $this->db_configure[$options] : $default;
                    break;
                case 'customize':
                    $this->feed_options[$options] = isset( $this->db_customize[$options] ) && ! empty( $this->db_customize[$options] ) ? $this->db_customize[$options] : $default;
                    break;
                default:
                    $this->feed_options[$options] = isset( $this->db_style[$options] ) && ! empty( $this->db_style[$options] ) ? $this->db_style[$options] : $default;
                    break;
            }
        }
    }
     ...
     * @param string $default       default value to use if there is no user input
     */
    public function setStandardTextOptions( $options, $options_page, $default = '' )
    {
        if ( is_array( $options ) ) {
            foreach ( $options as $option ) {
                switch ( $options_page ) {
                    case 'configure':
                        $this->feed_options[$option] = isset( $this->atts[$option] ) ? esc_attr( $this->atts[$option] ) : ( isset( $this->db_configure[$option] ) ? esc_attr( $this->db_configure[$option] ) : $default );
                        break;
                    case 'customize':
                        $this->feed_options[$option] = isset( $this->atts[$option] ) ? esc_attr( $this->atts[$option] ) : ( isset( $this->db_customize[$option] ) ?  esc_attr( $this->db_customize[$option] )  : $default );
                        break;
                    default:
                        $this->feed_options[$option] = isset( $this->atts[$option] ) ? esc_attr( $this->atts[$option] ) : ( isset( $this->db_style[$option] ) ?  esc_attr( $this->db_style[$option] )  : $default );
                        break;
                }
            }
        } else {
            switch ( $options_page ) {
                case 'configure':
                    $this->feed_options[$options] = isset( $this->atts[$options] ) ? esc_attr( $this->atts[$options] ) : ( isset( $this->db_configure[$options] ) ? esc_attr( $this->db_configure[$options] ) : $default );
                    break;
                case 'customize':
                    $this->feed_options[$options] = isset( $this->atts[$options] ) ? esc_attr( $this->atts[$options] ) : ( isset( $this->db_customize[$options] ) ?  esc_attr( $this->db_customize[$options] )  : $default );
                    break;
                default:
                    $this->feed_options[$options] = isset( $this->atts[$options] ) ? esc_attr( $this->atts[$options] ) : ( isset( $this->db_style[$options] ) ?  esc_attr( $this->db_style[$options] )  : $default );
                    break;
            }
        }
    }
    ...
    public function setTextSizeOptions( $options, $default = '' )
    {
        if ( is_array( $options ) ) {
            foreach ( $options as $option ) {
                $this->feed_options[$option] = isset( $this->atts[$option] ) ? $this->processTextSizeStyle( esc_attr( $this->atts[$option] ) ) : ( isset( $this->db_style[$option] ) ? $this->processTextSizeStyle( esc_attr( $this->db_style[$option] ) ) : $default );
            }
        } else {
            $this->feed_options[$options] = isset( $this->atts[$options] ) ? $this->processTextSizeStyle( esc_attr( $this->atts[$options] ) ) : ( isset( $this->db_style[$options] ) ? $this->processTextSizeStyle( esc_attr( $this->db_style[$options] ) ) : $default );
        }
    }
    ...
    public function setStandardStyleProperty( $options, $property, $default = '' )
    {
        if ( is_array( $options ) ) {
            foreach ( $options as $option ) {
                $this->feed_options[$option] = isset( $this->atts[$option] ) && $this->atts[$option] != 'inherit' ? $property . ': ' . esc_attr( $this->atts[$option] ) . ';'  : ( isset( $this->db_style[$option] ) && $this->db_style[$option] != '#' && $this->db_style[$option] != '' && $this->db_style[$option] != 'inherit' ? $property . ': ' . esc_attr( $this->db_style[$option] ) . ';' : $default );
            }
        } else {
            $this->feed_options[$options] = isset( $this->atts[$options] ) && $this->atts[$options] != 'inherit' ? $property . ': ' . esc_attr( $this->atts[$options] ) . ';'  : ( isset( $this->db_style[$options] ) && $this->db_style[$options] != '#' && $this->db_style[$options] != '' && $this->db_style[$options] != 'inherit' ? $property . ': ' . esc_attr( $this->db_style[$options] ) . ';' : $default );
        }
    }
     ...
     */
    public function setStandardBoolOptions( $options, $options_page, $default = true )
    {
        if ( is_array( $options ) ) {
            foreach ( $options as $option ) {
                switch ( $options_page ) {
                    case 'configure':
                        $this->feed_options[$option] = isset( $this->atts[$option] ) ? ( $this->atts[$option] === 'true' ) : ( isset( $this->db_configure[$option] ) ? (bool) $this->db_configure[$option] : (bool) $default );
                        break;
                    case 'customize':
                        $this->feed_options[$option] = isset( $this->atts[$option] ) ? ( $this->atts[$option] === 'true'  ) : ( isset( $this->db_customize[$option] ) ? (bool) $this->db_customize[$option] : (bool) $default );
                        break;
                    default:
                        $this->feed_options[$option] = isset( $this->atts[$option] ) ? ( $this->atts[$option] === 'true'  ) : ( isset( $this->db_style[$option] ) ? (bool) $this->db_style[$option] : (bool) $default );
                        break;
                }
            }
        } else {
    ...
    {
        $this->feed_options['width'] = isset( $this->atts['width'] ) ? 'width: '. esc_attr( $this->atts['width'] ) .';' : ( ( isset( $this->db_customize['width'] ) && $this->db_customize['width'] != '' ) ? 'width: '. esc_attr( $this->db_customize['width'] ) . ( isset( $this->db_customize['width_unit'] ) ? esc_attr( $this->db_customize['width_unit'] ) : '%' ) . ';' : '' );
        $this->feed_options['height'] = isset( $this->atts['height'] ) ? 'height: '. esc_attr( $this->atts['height'] ) .';' : ( ( isset( $this->db_customize['height'] ) && $this->db_customize['height'] != '' ) ? 'height: '. esc_attr( $this->db_customize['height'] ) . ( isset( $this->db_customize['height_unit'] ) ? esc_attr( $this->db_customize['height_unit'] ) : 'px' ) . ';' : '' );
    }
    ...
    {
        $user_cache = isset( $this->db_configure['cache_time'] ) ? ( $this->db_configure['cache_time'] * $this->db_configure['cache_time_unit'] ) : HOUR_IN_SECONDS;
    ...
    public function setFeedTypeAndTermOptions()
    {
        ...
        $this->feed_options['screenname'] = isset( $this->db_configure['usertimeline_text'] ) ? $this->db_configure['usertimeline_text'] : '';
        ...
        if ( isset( $this->atts['screenname'] ) ) {
            ...
            $this->feed_options['feed_term'] = isset( $this->atts['screenname'] ) ? ctf_validate_usertimeline_text( $this->atts['screenname'] ) : ( ( isset( $this->db_configure['usertimeline_text'] ) ) ? $this->db_configure['usertimeline_text'] : '' );
        ...
        if ( $this->feed_options['type'] == '' ) {
            ...
            switch ( $this->feed_options['type'] ) {
                case 'usertimeline':
                    $this->feed_options['feed_term'] = isset( $this->db_configure['usertimeline_text'] ) ? $this->db_configure['usertimeline_text'] : '';
                    break;
                case 'hometimeline':
                    $this->feed_options['type'] = 'hometimeline';
                    break;
                case 'search':
                    $this->feed_options['feed_term'] = isset( $this->db_configure['search_text'] ) ? $this->db_configure['search_text'] : '';
                    break;
            }
        }
    }
    ...
    public function setIncludeExcludeOptions()
    {
        ...
        if ( $this->feed_options['tweet_excludes'] == '' ) {
            $this->feed_options['tweet_includes'][] = isset( $this->db_customize['include_retweeter'] ) && $this->db_customize['include_retweeter'] == false ? null : 'retweeter';
            $this->feed_options['tweet_includes'][] = isset( $this->db_customize['include_avatar'] ) && $this->db_customize['include_avatar'] == false ? null : 'avatar';
            $this->feed_options['tweet_includes'][] = isset( $this->db_customize['include_author'] ) && $this->db_customize['include_author'] == false ? null : 'author';
            $this->feed_options['tweet_includes'][] = isset( $this->db_customize['include_text'] ) && $this->db_customize['include_text'] == false ? null : 'text';
            $this->feed_options['tweet_includes'][] = isset( $this->db_customize['include_date'] ) && $this->db_customize['include_date'] == false ? null : 'date';
            $this->feed_options['tweet_includes'][] = isset( $this->db_customize['include_actions'] ) && $this->db_customize['include_actions'] == false ? null : 'actions';
            $this->feed_options['tweet_includes'][] = isset( $this->db_customize['include_twitterlink'] ) && $this->db_customize['include_twitterlink'] == false ? null : 'twitterlink';
            $this->feed_options['tweet_includes'][] = isset( $this->db_customize['include_linkbox'] ) && $this->db_customize['include_linkbox'] == false ? null : 'linkbox';
        }
    }
    ...
    protected function trimTweetData()
    {
        ...
        for ( $i = 0; $i < $len; $i++ ) {
            ...
            $trimmed_tweets[$i]['user']['profile_image_url'] = $tweets[$i]['user']['profile_image_url'];
            ...
            if ( isset( $tweets[$i]['retweeted_status'] ) ) {
                ...
                $trimmed_tweets[$i]['retweeted_status']['user']['profile_image_url'] = $tweets[$i]['retweeted_status']['user']['profile_image_url'];
